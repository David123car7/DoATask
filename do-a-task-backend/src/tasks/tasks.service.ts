...
export class TasksService {
  ...
  async createTask(dto: CreateTasksDto, userId: string, imageName: string) {
    ...
    if (!community) {
      throw new HttpException(
        'Community with this name does not exist',
  ...
  async assignTask(userId: string, taskId: number) {
    ...
    if (member.userId == userId) {
      throw new HttpException(
        'The task creator cant accept the task',
        HttpStatus.BAD_REQUEST,
      );
    }
    ...
    if (!community) {
      throw new HttpException(
        'The task is not assigned to any community',
        HttpStatus.BAD_REQUEST,
      );
    }
    ...
    if (verify == false)
      throw new HttpException(
        'The user has allready accepted one task in this community',
        HttpStatus.BAD_REQUEST,
      );
    ...
    if (memberTask.volunteerId != null)
      throw new HttpException(
        'This task was allready accepted',
        HttpStatus.BAD_REQUEST,
      );
  ...
  async finishTask(memberTaskId: number) {
    ...
    try {
      ...
      this.notificationService.sendNotification(
        creator.userId,
        `Tarefa: ${task.title}`,
        `Tarefa ${task.title} foi terminada`,
      );
    } catch (error) {
  ...
  async evaluateTask(memberTaskId: number, score: number) {
    ...
    if (memberTask.status == TASK_STATES.EVALUATED) {
      throw new HttpException(
        'The task is allready evaluated',
        HttpStatus.BAD_REQUEST,
      );
    }
  ...
  async assignBonus(memberTaskId: number, score: number) {
    ...
    if (!memberTask) {
      throw new HttpException(
        'No memberTask found with the id',
        HttpStatus.BAD_REQUEST,
      );
    }
    ...
    if (!member) {
      throw new HttpException(
        'There is not member assigned to the task',
        HttpStatus.BAD_REQUEST,
      );
    }

    const { totalCoins, totalPoints } = this.calculateReward(
      memberTask.Task.difficulty,
      score,
    );
    ...
    if (!pointsMember) {
      throw new HttpException(
        'No pointsMember assigned to member',
        HttpStatus.BAD_REQUEST,
      );
    }
    ...
    try {
      const result = await this.prisma.$transaction(async () => {
        ...
        this.notificationService.sendNotification(
          member.userId,
          `Tarefa: ${task.title}`,
          `Tarefa ${task.title} foi avaliada`,
        );
      });
    } catch (error) {
  ...
  async GetTasksMemberDoing(userId: string) {
    ...
    if (!findMember) {
      throw new HttpException(
        'The user does not belong to any community',
        HttpStatus.BAD_REQUEST,
      );
    }
    ...
    if (!community) {
      throw new HttpException(
        'The member is not assigned to any community',
        HttpStatus.BAD_REQUEST,
      );
    }
    ...
    if (!tasks) {
      throw new HttpException(
        'The member is not assigned to member tasks',
        HttpStatus.BAD_REQUEST,
      );
    }
  ...
  async GetTasksMemberCreated(userId: string) {
    ...
    if (!member) {
      throw new HttpException(
        'The user does not belong to the any community',
        HttpStatus.BAD_REQUEST,
      );
    }
    ...
    if (!community) {
      throw new HttpException(
        'The member is not associated to the any community',
        HttpStatus.BAD_REQUEST,
      );
    }
    ...
    if (!tasks) {
      throw new HttpException(
        'The user has not created any tasks',
        HttpStatus.BAD_REQUEST,
      );
    }
    ...
    if (!memberTasks) {
      throw new HttpException(
        'There are memberTasks assigned to the tasks',
        HttpStatus.BAD_REQUEST,
      );
    }
    ...
    if (!tasksFiltered) {
      throw new HttpException(
        'There are not any task to be evaluated',
        HttpStatus.BAD_REQUEST,
      );
    }
    ...
    if (memberTasks.length == 0) {
      throw new HttpException(
        'There are not any task to evaluate',
        HttpStatus.BAD_REQUEST,
      );
    }
  ...
  async getTasksMemberDone(userId: string) {
    // USA string como nos outros
    const member = await this.prisma.member.findMany({
      where: { userId },
    });
  ...
  async getTaskBeDoneCommunity(communityName: string) {
    ...
    if (member.length == 0) {
      throw new HttpException(
        'The community does not have any members',
        HttpStatus.BAD_REQUEST,
      );
    }
    ...
    const storage = this.supabaseService
      .getAdminClient()
      .storage.from(BUCKETS.TASK_IMAGES);
    if (!storage) {
      throw new HttpException(
        'The bucket storage does not exist',
        HttpStatus.BAD_REQUEST,
      );
    }

    const tasksWithImages = await Promise.all(
      taskFilter.map(async (task) => {
  ...
  async DeleteTask(taskId: number) {
    ...
    if (!task) {
      throw new HttpException(
        'There is not a task with this id',
        HttpStatus.BAD_REQUEST,
      );
    }
    ...
    if (!memberTask) {
      throw new HttpException(
        'The task is not assigned to a memberTask',
        HttpStatus.BAD_REQUEST,
      );
    }
    ...
    if (memberTask.status != TASK_STATES.NOT_ACCEPTED)
      throw new HttpException(
        'The task was allready accepted',
        HttpStatus.BAD_REQUEST,
      );
  ...
  async CancelTask(taskId: number) {
    ...
    if (!memberTask) {
      throw new HttpException(
        'The task is not assigned to a member task',
        HttpStatus.BAD_REQUEST,
      );
    }
    ...
    if (memberTask.status != TASK_STATES.ACCEPTED)
      throw new HttpException(
        'The task can only be canceled when its not finished',
        HttpStatus.BAD_REQUEST,
      );
    ...
    try {
      const result = await this.prisma.$transaction(async () => {
        ...
        this.notificationService.sendNotification(
          creator.userId,
          `Tarefa: ${task.title}`,
          `Tarefa ${task.title} foi cancelada`,
        );
      });
    } catch (error) {
