...
export async function UpdateStock(stock: number, itemId: number) {
  try {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/store/updateStock?stock=${stock}&itemId=${itemId}`, 
    {
        method: 'PUT',
        headers: {
