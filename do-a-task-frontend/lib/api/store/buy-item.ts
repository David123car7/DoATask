...
export async function BuyItem(itemId: number,communityName: string) {
  try {
    const access_token = await getCookie(AUTH_COOKIES.ACCESS_TOKEN);
    if (!access_token) {
      return { message: "Access token not found", state: false };
    }
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/store/buyItem?itemId=${itemId}&communityName=${communityName}`,
    {
        method: 'PUT',
        headers: {
